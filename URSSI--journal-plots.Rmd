...
```{r Q1, cache=TRUE, hide = TRUE, include = FALSE}
...
```{r Q2_2, cache=TRUE, include = FALSE}
...
```{r Q2_2_details, cache=TRUE, include = FALSE}
...
```{r Q2_3_states, cache=TRUE, include = FALSE}
...
```{r Q2_3map, cache=TRUE, include = FALSE}
...
```{r Q2_3, cache=TRUE, include=FALSE}
...
```{r Q2_3_details, cache=TRUE, include=FALSE}
...
```{r Q2_4, cache=TRUE, include=FALSE}
dat = dat_na %>%
...
```{r Q2_4_details, include=FALSE, message = F, show = FALSE}
...
<!-- ## Figure 1a: What percent of your time did you actually spend in the following activities (total should be 100%)?  -->

```{r fig1, show = FALSE, echo = FALSE, hide = TRUE }
...
```{r Q3_4, include=FALSE, cache = TRUE}
dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
...
# FIGURE 1
# TODO
# Change this to a likert scale figure if possible
# Also label the axes as needed
dat1 = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
    select(Q1.1, Q3.3_1, Q3.3_2, Q3.3_3, Q3.3_4, Q3.3_5, Q3.3_6, Q3.3_7, Q3.3_8, Q3.3_9, Q3.3_10)
...
# Changed Software to Design per Jeff's comment in issue 1
# 
datm$Question <- recode_factor(datm$Question, Software = "Design")
foo <- levels(datm$Question) 
datm$Question <- forcats::fct_relevel(datm$Question, "Requirements", "Design")
library(viridis)
 figure1 <- datm %>%
 ggplot(aes(x = Q1.1, fill = Time, y = value)) + 
  facet_wrap(vars(Question, Q1.1) ) +
       scale_fill_viridis(discrete = TRUE) +
  geom_boxplot(fatten = NULL, outlier.size = .5) +
    stat_summary(fun = mean, geom = "errorbar",
                 aes(ymax = ..y.., ymin = ..y..),
                 width = .75, linetype = "solid", position = "dodge") +
  ...
  labs(title =str_wrap("Are Respondents Spending More Time On Tasks Than They Would Like Ideally?", 70),
       y = "Percent of time") 
figure1
ggsave(file="~/Desktop/urssi_figures/svg/figure_1.svg", plot=figure1, width=10, height=8)
```
...
## Figure 3: Which of the following types of testing did you employ?
...
figure3 <- ggplot(datmm, aes(x = Question, fill = Choice)) +
    ...
    theme(axis.text.x = element_text(angle = 35, hjust = 1),
          legend.title = element_blank())
figure3
ggsave(file="~/Desktop/urssi_figures/svg/figure_3.svg", plot=figure3, width=10, height=8)
# ggplot(datmm, aes(x = Question, fill = Choice)) +
#     facet_wrap('Q1.1') + 
#     geom_bar(position = 'fill') + 
#     labs(x = '', y = 'Frequency') +
...
```{r show = FALSE, echo = FALSE, cache = TRUE}
...
```{r show = FALSE, cache = TRUE, echo = FALSE}
# dat %>%
#   group_by(Q1.1, Q3.6) %>%
#   tally() %>%
#   left_join(tmp, by = "Q1.1") %>%
...
## Figure 4: How well do current tools support each of the following activities? 
...
figure4
ggsave(file="~/Desktop/urssi_figures/svg/figure_4.svg", plot=figure4, width=10, height=8)
# ggplot(datmm, aes(x= Question, fill = Choice)) +
#     geom_bar(position = 'fill') + 
#     labs(x = '', y = 'Proportion') +
#     theme(axis.text.x = element_text(angle = 30, hjust = 1),
...
```{r Q5_3_g, show = FALSE, cache = TRUE }
dat = dat_na %>%
...
```{r Q5_3_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
```{r fig10_related, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
## Figure 6: Do you have sufficient time to take the types of training you need to be successful?
...
figure6 <- ggplot(dat, aes(x = Q1.1, fill = Q5.5)) +
    ...
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          legend.title = element_blank())
figure6
ggsave(file="~/Desktop/urssi_figures/svg/figure_6.svg", plot=figure6, width=10, height=8)
...
<!-- ## Also FIGURE 6: For each of the following types of training (languages, development techniques, and project management), which mode of delivery do you prefer? Please select your top two modes of delivery preferences for each of the training types. -->
...
```{r cache=FALSE, show=FALSE, message = FALSE}
# datmmm %>%
...
```{r Q5_6_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(!is.na(Gender)) %>%
...
```{r Q5_6_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
```{r Q6_3_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
```{r Q6_3_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
## Figure 7: What percent of the funding for your software work comes from each of the following?
...
figure7
ggsave(file="~/Desktop/urssi_figures/svg/figure_7.svg", plot=figure7, width=10, height=8)
```
...
```{r Q6_4_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
...
```{r Q6_4_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
...
## Figure 8: Does your institution provide support for your research software development activities in the following ways
...
figure8 <- ggplot(datmm, aes(x= Question, fill = Choice)) +
    ...
    guides(fill=guide_legend(nrow=2,byrow=TRUE))
figure8
ggsave(file="~/Desktop/urssi_figures/svg/figure_8.svg", plot=figure8, width=10, height=8)
# 
# ggplot(datmm, aes(x= Question, fill = Choice)) +
#     geom_bar(position='fill') + 
#     labs(x = '', y = 'Proportion') +
...
```{r Q6_5_g, show = FALSE, cache = TRUE}
dat = dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
...
```{r Q6_5_c, show = FALSE, cache = TRUE}
dat = dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Developers') %>%
...
```{r Q6_6_g, show = FALSE, cache = TRUE}
dat = dat = dat_na %>%
    filter(Q1.1 == 'Researchers') %>%
...
```{r Q6_6_c, show = FALSE, cache = TRUE }
dat = dat = dat_na %>%
    filter(Q1.1 == 'Researchers') %>%
...
```{r Q7_2_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
```{r Q7_2_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
...
dat_na$Q7.3 = ifelse(dat_na$Q7.3=='No - I would have to move to another institution', 'No', 
                  ifelse(dat_na$Q7.3=="Don’t know", "Don't know",
...
## FIGURE 10: Is there opportunity for career advancement (e.g., tenure, more senior positions) for people whose primary role is to develop research software at your institution?
...
```{r show = FALSE, cache = TRUE}
# This is figure 10
figure10 <- urssi_analysis2(dat_na, x = "Q1.1", fill = "Q7.3")  + scale_fill_viridis(discrete = TRUE) 
figure10
ggsave(file="~/Desktop/urssi_figures/svg/figure_10.svg", plot=figure10, width=10, height=8)
```
...
## Figure 11: With regard to hiring and maintaining a highly qualified software development staff, how important are the following concerns?
...
figure11 <- ggplot(datmm, aes(x= Q1.1, fill = Choice)) +
    ...
    guides(fill=guide_legend(nrow=2,byrow=TRUE))
figure11
ggsave(file="~/Desktop/urssi_figures/svg/figure_11.svg", plot=figure11, width=10, height=8)
...
```{r Q7_4_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Researchers') %>%
...
```{r Q7_4_c, show = FALSE, cache = TRUE}
dat = dat = dat_na %>%
    filter(Q1.1== 'Combination' | Q1.1 == 'Researchers') %>%
...
## Figure 12: In your opinion: How important were the following concerns when you were hired into your current position?
...
figure12 <- ggplot(datmm, aes(x= Q1.1, fill = Choice)) +
    ...
    guides(fill=guide_legend(nrow=2,byrow=TRUE))
figure12
ggsave(file="~/Desktop/urssi_figures/svg/figure_12.svg", plot=figure12, width=10, height=8)
...
## Figure 14: When you write a paper and the work being described uses software, how often do you use each of the following approaches to mention the software? - Cite paper about the software
...
figure14 <- datmm %>% filter(Question != "Other")  %>% ggplot(aes(x= Q1.1, fill = Choice)) +
...
figure14
ggsave(file="~/Desktop/urssi_figures/svg/figure_14.svg", plot=figure14, width=10, height=8)
...
```{r Q8_2_g, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(!is.na(Gender)) %>%
...
```
## Figure 15: How often do you currently receive the following types of credit for your software contributions?
...
figure15
ggsave(file="~/Desktop/urssi_figures/svg/figure_15.svg", plot=figure15, width=10, height=8)
...
#     fix_border_issues()
```
## Figure 16: For your job role, does your institution allow software contributions to be considered in performance reviews or promotion cases?
...
figure16 <- ggplot(dat, aes(x= Q1.1, fill = Q8.4)) +
    ...
    theme(axis.text.x = element_text(angle = 40, hjust = 1),
          ...
          legend.position = 'bottom') #+ guides(fill=guide_legend(nrow=2,byrow=TRUE))
figure16
ggsave(file="~/Desktop/urssi_figures/svg/figure_16.svg", plot=figure16, width=10, height=8)
...
## Figure 17: How important do you believe software contributions are for your own performance review or promotion case?
...
figure17 <- ggplot(dat, aes(x= Q1.1, fill = Q8.5)) +
    ...
    guides(fill=guide_legend(nrow=2,byrow=TRUE))
figure17
ggsave(file="~/Desktop/urssi_figures/svg/figure_17.svg", plot=figure17, width=10, height=8)
...
## Figure 18: How well does your project do the following
...
figure18
ggsave(file="~/Desktop/urssi_figures/svg/figure_18.svg", plot=figure18, width=10, height=8)
...
```{r Q9_2_g,  show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(!is.na(Gender)) %>%
...
```{r Q9_2_c, show = FALSE, cache = TRUE}
dat = dat_na %>%
    filter(!is.na(CareerStage)) %>%
...
### Figure 2: Overall, regardless of where you spend the most time, which of the following aspects of software development are more difficult than you think they should be?
...
figure2
ggsave(file="~/Desktop/urssi_figures/svg/figure_2.svg", plot=figure2, width=10, height=8)
...
### Figure 9: In general, do you think you have sufficient funding to support software development activities in your research?
```{r fig9}
dat = dat_na %>%
...
figure9
ggsave(file="~/Desktop/urssi_figures/svg/figure_9.svg", plot=figure9, width=10, height=8)
...
### Figure 13: Rate the importance of each of the following factors when deciding upon accepting a new job position or promotion.
...
figure13
ggsave(file="~/Desktop/urssi_figures/svg/figure_13.svg", plot=figure13, width=10, height=8)
...
#     border_inner_h(border = officer::fp_border(color = "black", width = 1), part = "body" ) %>%
#     fix_border_issues()
#     print("This is where a table shows")
```

<!-- ## Diversity and Inclusion Follow Up --> 

### Figure 19: Considering the software project that you spend the most time working on, does this project have a diversity / inclusion statement or policy?
...
figure19 <- ggplot(dat, aes(x= Q17.1, fill = Q1.1)) +
    ...
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          ...
          legend.title = element_blank())
figure19
ggsave(file="~/Desktop/urssi_figures/svg/figure_19.svg", plot=figure19, width=10, height=8)
...
### Figure 20: Considering the software project that you spend the most time working on or with, does this project have a code of conduct?
...
figure20 <- ggplot(dat, aes(x= Q17.3, fill = Q1.1)) +
    ...
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
          ...
          legend.title = element_blank())
figure20
ggsave(file="~/Desktop/urssi_figures/svg/figure_20.svg", plot=figure20, width=10, height=8)
...
```{r  cache = TRUE, echo = FALSE, show = FALSE}
# dat %>%
#     group_by(Q1.1, Q17.3) %>%
#     tally() %>%
#     left_join(tmp3, by = "Q1.1") %>%
...
### Figure 21: Which of the following inclusive practices and policies do you need support in creating for projects that focus on or include software development? Please mark all that apply.
...
figure21
ggsave(file="~/Desktop/urssi_figures/svg/figure_21.svg", plot=figure21, width=10, height=8)
